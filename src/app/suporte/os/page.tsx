      ...
      <div className="space-y-6 relative">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm font-bold text-gray-700">Filtrar por Data:</span>
            <div className="flex items-center gap-2">
              ...
              {(dateFilter.start || dateFilter.end) && (
                <button
                  ...
                  className="px-3 py-2 bg-red-50 text-red-600 rounded-lg text-sm font-bold hover:bg-red-100 transition-all"
                >
                  Limpar
                </button>
              )}
        ...
        {loading ? <p>Carregando...</p> : (
            ...
            {ordensServico
              .filter(os => {
                // Status filter
                const statusMatch = statusFilter === "todos" || os.status === statusFilter;
                
                // Date filter
                if (!dateFilter.start && !dateFilter.end) return statusMatch;
                
                const osDate = new Date(os.dataInicio || "");
                const startDate = dateFilter.start ? new Date(dateFilter.start) : null;
                const endDate = dateFilter.end ? new Date(dateFilter.end) : null;
                
                let dateMatch = true;
                if (startDate && osDate < startDate) dateMatch = false;
                if (endDate && osDate > endDate) dateMatch = false;
              ...
              <div key={os.id} onClick={() => handleEdit(os)} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow cursor-pointer group">
                ...
                {os.dataInicio && (
                  <div className="flex items-center gap-1 text-xs text-blue-600 mb-2">
                    ...
                    <span>{new Date(os.dataInicio).toLocaleDateString('pt-BR')}</span>
                  </div>
                )}
